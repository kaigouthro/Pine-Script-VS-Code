import { Helpers } from './index' // VSCode removed
...
export class PineDocString {
  ...
  async docstring(): Promise<void> {
    ...
    const selection = activeEditor.selection; // Replaced VSCode?.Selection
    if (!selection) {
      return
    }
    // Replaced VSCode?.SelectedText
    ...
    activeEditor.edit((editBuilder) => { // Replaced VSCode.Editor?.edit
